...
function translateNorwegian(text) {
  let t = text;
  for (const [re, en] of NO_TO_EN)
    t = t.replace(re, typeof en === 'function' ? (m, ...args) => en(m, ...args) : en);
  return t;
}
...
function detectAfterCompletion(text) {
  ...
  if (m3) {
    return {
      xRecurringType: 'after-completion',
      xRecurringInterval: /week/i.test(m3[1]) ? 'weekly' : 'daily',
      matchText: m3[0],
    };
  }
  ...
  if (m4) {
    return {
      xRecurringType: 'after-completion',
      xRecurringInterval: /week/i.test(m4[1]) ? 'weekly' : 'daily',
      matchText: m4[0],
    };
  }
  return null;
...
function parseTask(text, refDate = new Date(), timezone = 'UTC') {
  ...
  if (afterComp) {
    ...
    for (const [re] of RRULE_TASK) {
      ...
      if (m) {
        recurrenceMatchText = m[0];
        forParsing = forParsing.replace(m[0], '').trim();
        break;
      }
    }
  }
  ...
  if (recurrenceMatchText) {
    ...
    const origMinusRec = trimmed
      .replace(new RegExp(escapeRegex(recurrenceMatchText), 'i'), '')
      .trim();
