...
const skipAndroidTests = process.env.NODECAL_SKIP_ANDROID_TESTS === '1';

test(
  'Android launcher includes a complete set of dated icon aliases',
  { skip: skipAndroidTests },
  () => {
    ...
    for (let day = 1; day <= 31; day += 1) {
      ...
      assert.ok(
        fs.existsSync(
          path.join(
            root,
            `android/app/src/main/res/mipmap-anydpi-v26/ic_launcher_day_${value}.xml`,
          ),
        ),
      );
    }
...
test(
  'Android starts with contrasting system bar icons on the light header',
  { skip: skipAndroidTests },
  () => {
